  ...
  return(
    <div style={{background:"#0a0f1a",border:"1px solid #161f35",borderRadius:10,padding:"12px 14px"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
        ...
        <button onClick={onDelete} style={{background:"none",border:"none",color:"#283350",cursor:"pointer",fontSize:13}}>✕</button>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"6px 12px",fontSize:11.5}}>
        ...
        <div><span style={{color:"#374460"}}>수익 </span><span style={{color:pc(r),fontFamily:"monospace",fontWeight:700}}>{pct(r)}</span></div>
      </div>
      <div style={{marginTop:6,textAlign:"right",fontSize:12,color:pc(pl),fontFamily:"monospace",fontWeight:700}}>{(pl>=0?"+":"")+fmt(pl)}원</div>
    </div>
  );
}
    ...
    <div style={{minHeight:"100vh",background:"#060a12",color:"#a0adc4",fontFamily:"'Pretendard','Inter',sans-serif"}}>
        ...
        <div className="header-right">
          ...
          <div className="tab-bar">
            {["📊 포트폴리오","🔍 종목분석","💡 AI추천","🌐 시장스캔"].map((t,i)=>(
              <button key={i} onClick={()=>setTab(i)} className={`tab-btn${tab===i?" active":""}`}>{t}</button>
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="main-content">
        ...
        {tab===0&&(<div style={{display:"flex",flexDirection:"column",gap:14}}>
          ...
          <Card title="보유 종목">
            {/* Desktop Table */}
            <div className="desktop-table" style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}><thead><tr style={{borderBottom:"1px solid #161f35"}}>{["종목","섹터","수량","평균단가","현재가","수익률","평가손익",""].map((h,i)=>(<th key={i} style={{padding:"8px 4px",textAlign:i>=2?"right":"left",color:"#374460",fontWeight:600,fontSize:10.5}}>{h}</th>))}</tr></thead>
            ...
            <div className="mobile-cards">
              {pf.map((p,i)=>(<MobileStockCard key={i} p={p} i={i} onDelete={()=>setPf(pf.filter((_,j)=>j!==i))}/>))}
            </div>
        ...
        {tab===1&&(<div style={{display:"flex",flexDirection:"column",gap:14}}>
          <Card>
            ...
            <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
              <input value={q} onChange={e=>setQ(e.target.value)} onKeyDown={e=>e.key==="Enter"&&analyze()} placeholder="예: 삼성전자, SK하이닉스, 테슬라..." style={{...IS,flex:1,fontSize:14,padding:"10px 15px",minWidth:0}}/>
          ...
          {(ld||res)?(<Card style={{minHeight:160}}>{ld?<LoadDots msg="웹에서 최신 정보를 검색하고 분석하는 중"/>:<RenderAI text={res}/>}</Card>):(
            <div className="quick-grid">{["삼성전자","SK하이닉스","NAVER","카카오","현대차","셀트리온"].map(n=>(<button key={n} onClick={()=>setQ(n)} style={{background:"#090e18",border:"1px solid #161f35",borderRadius:8,padding:"12px 13px",color:"#5e6e88",fontSize:12.5,cursor:"pointer",textAlign:"left"}}><span style={{color:"#7c5cfc",marginRight:6}}>→</span>{n} 분석하기</button>))}</div>
          )}
        </div>)}
        ...
        {tab===3&&(<div style={{display:"flex",flexDirection:"column",gap:14}}>
          <Card>
            ...
            <div style={{marginBottom:14}}><div style={{fontSize:11,color:"#374460",marginBottom:8,fontWeight:600}}>🔥 인기 테마 선택</div>
              <div className="themes-grid">{THEMES.map(t=>(<button key={t.k} onClick={()=>scan(t.k)} disabled={sL} style={{background:sT===t.k&&(sL||sR)?"linear-gradient(135deg,#7c5cfc18,#00e4a008)":"#070b14",border:sT===t.k&&(sL||sR)?"1px solid #7c5cfc33":"1px solid #161f35",borderRadius:8,padding:"10px 8px",cursor:sL?"default":"pointer",opacity:sL?0.5:1,textAlign:"center",fontFamily:"inherit"}}><div style={{fontSize:20,marginBottom:3}}>{t.e}</div><div style={{fontSize:11.5,color:"#8a99b5",fontWeight:600}}>{t.l}</div></button>))}</div>
            </div>
            <div><div style={{fontSize:11,color:"#374460",marginBottom:7,fontWeight:600}}>✏️ 직접 입력</div>
              <div style={{display:"flex",gap:8,flexWrap:"wrap"}}><input value={sC} onChange={e=>setSC(e.target.value)} onKeyDown={e=>e.key==="Enter"&&scan()} placeholder="예: 트럼프 수혜주, 금리 인하, K-뷰티..." style={{...IS,flex:1,fontSize:13.5,padding:"10px 14px",minWidth:0}}/><button onClick={()=>scan()} disabled={sL||!sC.trim()} style={{...BP,opacity:(sL||!sC.trim())?0.5:1,whiteSpace:"nowrap"}}>{sL?"검색 중...":"🔍 스캔"}</button></div>
            </div>
          </Card>
          {(sL||sR)?(<Card style={{minHeight:160}}>{sL?<LoadDots msg={`"${sT}" 관련 최신 뉴스와 종목을 검색하는 중`}/>:<div><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:14,padding:"10px 14px",background:"#070b14",borderRadius:8,border:"1px solid #161f35",flexWrap:"wrap"}}><span style={{fontSize:16}}>🌐</span><span style={{fontSize:13,color:"#7c5cfc",fontWeight:700}}>검색 테마:</span><span style={{fontSize:13,color:"#d5dced",fontWeight:600}}>{sT}</span></div><RenderAI text={sR}/></div>}</Card>):(
